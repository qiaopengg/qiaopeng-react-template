...
// 创建 Context 来共享 Select 的 value
const SelectContext = React.createContext<{
  value?: string;
}>({});

function Select({ value, children, ...props }: React.ComponentPropsWithoutRef<typeof SelectPrimitive.Root>) {
...
function SelectTrigger({
  ...
  const handleKeyDown = (e: React.KeyboardEvent<HTMLDivElement>) => {
    if (e.key === "Enter" || e.key === " ") {
      ...
      handleClear(mockMouseEvent);
    }
  };

  const showClearButton = React.useMemo(() => {
    return clearable && value && value !== "";
  }, [clearable, value]);
      ...
      <div className="flex items-center">
        {showClearButton && (
          <div
            ...
            className="pointer-events-auto flex items-center justify-center rounded-sm p-0.5 hover:bg-muted/50 transition-colors cursor-pointer"
            aria-label="清空选择"
            tabIndex={0}
          >
            <X className="size-3.5 opacity-50 hover:opacity-70" />
          </div>
        )}
        {!showClearButton && (
